      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-4">
          {selectedTickets.map((ticket) => {
            const isSoldOut = ticket.max - ticket.soldNumber > 0
            return (
              <div
                key={ticket.ticket_id}
                className={`flex justify-between gap-4 items-center px-6 py-4 rounded-2xl ${isSoldOut ? "bg-[#1b1b1b] text-[#FAFAFA]" : "border border-[#B2BCC2] text-[#1b1b1b]"
                  }`}
              >
                <div>
                ...
                {isSoldOut ? (
                  <div className="flex justify-center items-center text-[#FAFAFA] text-xl font-bold">
                    <button
                      className={`w-12 h-12 rounded-full flex justify-center items-center ${ticket.amount <= 0
                    ...
                    <button
                      className={`w-12 h-12 rounded-full flex justify-center items-center ${ticket.amount >= ticket.max
                        ? "bg-gray-400 cursor-not-allowed"
